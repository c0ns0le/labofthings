...
namespace HomeOS.Hub.Drivers.Dummy2
{
    ...
    public class DriverDummy2 :  ModuleBase
    {
        ...
        public override void Start()
        {
            ...
            List<VRole> listRole = new List<VRole>() { RoleDummy2.Instance };
            BindRoles(dummyPort, listRole);

            //.................register the port after the binding is complete
            RegisterPortWithPlatform(dummyPort);

            workThread = new SafeThread(delegate() { Work(); } , "DriverDummy2 work thread" , logger);
            workThread.Start();
        ...
        public void Work()
        {
            ...
            while (true)
            {
                ...
                Notify(dummyPort, RoleDummy2.Instance, RoleDummy2.OpEchoSubName, new ParamType(counter));
        ...
        public override IList<VParamType> OnInvoke(string roleName, String opName, IList<VParamType> args)
        {
            ...
            if (!roleName.Equals(RoleDummy2.RoleName))
            {
        ...
        ///        the dummy2 driver does not care about other ports in the system
        /// </summary>
